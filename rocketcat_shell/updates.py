...
class UpdateService:
    ...
    def official_asset_name(tag_name: str) -> str:
        return f"RocketCatShell-{tag_name}.zip"
    ...
    async def prepare_switch(
        ...
        *,
        ...
        if os.name != "nt":
            raise UpdatePackageError("the v0.2.2 updater supports Windows only")
        ...
        async with self._lock:
            ...
            if release is None:
                raise UpdatePackageError(
                    "target release is unavailable or lacks the Windows asset"
                )
