...
- Windows live 暂时内置 `rocketcat_plugin_dashboard_demo` 作为 UI 与 Bridge 验收插件；它不处理任何 Bot 消息。
...
- `rocketcat_plugin_dashboard_demo`：v0.2.1 Windows live 验收插件。它没有 `main.py`，不处理 Bot 消息，只用于验证纯 Dashboard 插件的页面发现、全屏 UI 和 Bridge 上下文。
