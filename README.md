...
- E2EE 图片解密后会进入当前 Bot 数据目录下的受控媒体缓存，并通过仅监听 `127.0.0.1`、使用随机能力令牌的 WebUI HTTP 地址交给 OneBot 上游；旧消息快照里的系统临时路径或 `base64://` 引用也会在 `get_msg` 时重新发布。这样 `/生图` 与 `/grok生图` 接收到的参考图来源和非加密频道一致，不再依赖 AstrBot 接受 RocketCatShell 的本地 `%TEMP%` 路径。
