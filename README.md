...
> 当前 README 对应版本为 `v0.2.0`。本版在保持 v0.1.9 身份、媒体、E2EE 与 Rocket.Chat 兼容行为不变的前提下，集中优化入站热路径、身份映射、媒体缓存、资源边界和 runtime 重载。
...
|------|------|
| Python | `>= 3.11` |
| 运行依赖 | `aiohttp`, `cryptography`, `fastapi`, `orjson`, `psutil`, `python-multipart`, `uvicorn`；Windows 额外使用 `pywinpty` |
