...
> 当前 README 对应版本为 `v0.1.9`。`v0.1.3` 是破坏性架构重构基线，`v0.1.4` 统一收口性能优化，`v0.1.5` 补齐了内置指令与运维增强，`v0.1.6` 推进运行诊断可观测性和入站热路径性能收口，`v0.1.7` 开始补齐面向 Docker 迁移的内置文件管理能力，`v0.1.8` 引入 NapCat 风格的 WebUI 系统终端，而 `v0.1.9` 完善 Rocket.Chat 多引用消息的原生 OneBot 回传能力，并补齐 Rocket.Chat 7.10–8.5 的接口兼容。
...
- E2EE 图片解密后会进入当前 Bot 数据目录下的受控媒体缓存，并通过仅监听 `127.0.0.1`、使用随机能力令牌的临时 HTTP 地址交给 OneBot 上游；旧消息快照里的系统临时路径或 `base64://` 引用也会在 `get_msg` 时重新发布。这样 `/生图` 与 `/grok生图` 接收到的参考图来源和非加密频道一致，不再依赖 AstrBot 接受 RocketCatShell 的本地 `%TEMP%` 路径。
...
- E2EE 解密媒体不会再把系统 `%TEMP%` 绝对路径直接交给上游；RocketCatShell 会在本机回环地址提供带随机令牌的临时 HTTP 媒体 URL，并在读取旧 `get_msg` 缓存时修复遗留路径或 Base64 引用，兼容 AstrBot 生图插件的安全目录策略。
...
|------|------|
| Python | `>= 3.11` |
| 运行依赖 | `aiohttp`, `cryptography`, `fastapi`, `orjson`, `psutil`, `python-multipart`, `PyYAML`, `uvicorn`, `websockets`；Windows 额外使用 `pywinpty` |
